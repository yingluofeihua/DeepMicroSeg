...
class ModelParameterAnalyzer:
    ...
    def analyze_sam_model_parameters(self, model_type="vit_l_lm", checkpoint_path=None, 
                                   freeze=None, peft_kwargs=None, device=None) -> Dict:
        """
        分析SAM模型参数
        
        Args:
            model_type: 模型类型 (vit_b, vit_l_lm, vit_l, vit_h等)
...
# 使用示例函数
def analyze_training_model_parameters(model_type="vit_l_lm", checkpoint_path=None, 
    ...
    使用示例:
    # 基本分析
    analyze_training_model_parameters("vit_l_lm", logger=logger)
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            if self.with_segmentation_decoder and self.label_transform is not None:
                # 使用官方的PerObjectDistanceTransform
                # 这会自动生成4通道输出：distances, boundary_distances, foreground, instances
                transformed_mask = self.label_transform(mask)
                
                # transformed_mask应该已经是正确的4通道格式
                ...
                if len(mask_tensor.shape) == 2:
                    ...
                    mask_4ch[0] = mask_tensor  # instances
                    mask_4ch[1] = (mask_tensor > 0).float()  # foreground
            ...
            if self.with_segmentation_decoder:
                return (torch.ones(1, 512, 512, dtype=torch.float32) * 128.0,
...
class OptimizedDatasetHandler:
    ...
    def create_official_dataloaders(self, patch_infos_train, patch_infos_val, batch_size=2):
        """创建正确的4通道标签数据加载器"""
        self.logger.log_info("Creating 4-channel compatible micro_sam dataloaders...")
        ...
        class MicroSAM4ChannelDataset(Dataset):
            """确保4通道标签格式的数据集，严格验证前景对象"""
            
            def __init__(self, patch_infos, logger, is_train=True):
                self.patch_infos = patch_infos
                self.logger = logger
                self.is_train = is_train
                ...
                # 预验证数据 - 更严格的验证
                self.valid_indices = []
                self.valid_samples = []  # 存储验证过的样本信息
                
                self.logger.log_info(f"Pre-validating {len(patch_infos)} patches with strict foreground checks...")
                ...
                for i, patch_info in enumerate(patch_infos):
                    try:
                        ...
                        # 基本形状检查
                        if not (len(img.shape) == 2 and len(mask.shape) == 2 and 
                            img.shape == mask.shape == (512, 512)):
                            continue
                        ...
                        # 严格的前景对象验证
                        valid_sample = self._validate_foreground_objects(mask, img_path)
                        if valid_sample:
                            self.valid_indices.append(i)
                            self.valid_samples.append(valid_sample)
                            
                    except Exception as e:
                        self.logger.log_warning(f"Validation failed for {patch_info.get('img_path', 'unknown')}: {e}")
                        continue
                
                self.logger.log_info(f"MicroSAM4ChannelDataset: {len(self.valid_indices)}/{len(patch_infos)} valid patches")
                
                if len(self.valid_indices) == 0:
                    raise ValueError("No valid patches found with foreground objects!")
                
                # 统计验证结果
                total_objects = sum(sample['num_objects'] for sample in self.valid_samples)
                avg_objects = total_objects / len(self.valid_samples) if self.valid_samples else 0
                self.logger.log_info(f"Total foreground objects: {total_objects}, Average per patch: {avg_objects:.1f}")
            
            def _validate_foreground_objects(self, mask, img_path):
                """严格验证前景对象"""
                try:
                    # 检查基本前景
                    ...
                    # 计算连通组件（实例）
                    instances = measure.label(mask > 0)
                    unique_instances = np.unique(instances)[1:]  # 跳过背景
                    
                    if len(unique_instances) == 0:
                        self.logger.log_warning(f"No instances found in {img_path}")
                        return None
                    ...
                    for instance_id in unique_instances:
                        ...
                        # 过滤太小的实例（至少25像素，与PerObjectDistanceTransform的min_size一致）
                        if instance_size >= 25:
                    ...
                    if len(valid_instances) == 0:
                        self.logger.log_warning(f"No valid instances (size >= 25) found in {img_path}")
                        return None
                    
                    # 检查实例标签连续性
                    max_label = mask.max()
                    unique_labels = len(np.unique(mask)) - 1  # 减去背景
                    
                    return {
                        'num_objects': len(valid_instances),
                        'foreground_pixels': foreground_pixels,
                    ...
                    self.logger.log_warning(f"Foreground validation failed for {img_path}: {e}")
                    return None
            ...
            def __getitem__(self, idx):
                ...
                try:
                    ...
                    # 再次验证这个样本确实有前景对象
                    if np.sum(mask > 0) < 50:
                        self.logger.log_warning(f"Sample {idx} lost foreground objects, using backup")
                        return self._get_backup_sample()
                    ...
                    try:
                        ...
                        # 验证4通道格式
                        if len(transformed_mask.shape) == 3 and transformed_mask.shape[0] == 4:
                            mask_4ch = transformed_mask.astype(np.float32)
                            
                            # 验证变换后的标签仍有前景
                            if not self._validate_transformed_mask(mask_4ch):
                                self.logger.log_warning(f"Transformed mask lost foreground for {img_path}")
                                mask_4ch = self._create_4channel_manual(mask, sample_info)
                        else:
                            mask_4ch = self._create_4channel_manual(mask, sample_info)
                            
                    except Exception as e:
                        self.logger.log_warning(f"Label transform failed for {img_path}: {e}")
                        mask_4ch = self._create_4channel_manual(mask, sample_info)
                    
                    # 转换为张量
                    img_tensor = torch.from_numpy(img).unsqueeze(0).float()  # (1, H, W)
                    mask_tensor = torch.from_numpy(mask_4ch).float()  # (4, H, W)
                    
                    # 最终验证
                    assert img_tensor.shape == (1, 512, 512), f"Image shape: {img_tensor.shape}"
                    assert mask_tensor.shape == (4, 512, 512), f"Mask shape: {mask_tensor.shape}"
                    
                    # 验证前景通道不为空
                    foreground_channel = mask_tensor[2]  # 前景通道
                    if torch.sum(foreground_channel > 0) < 25:
                        self.logger.log_warning(f"Final validation failed for {img_path}, using backup")
                        return self._get_backup_sample()
            ...
            def _validate_transformed_mask(self, mask_4ch):
                """验证变换后的4通道掩码"""
                # 检查前景通道
                foreground_channel = mask_4ch[2]
                if np.sum(foreground_channel > 0) < 25:
                    return False
                
                # 检查实例通道
                instance_channel = mask_4ch[3]
                if np.sum(instance_channel > 0) < 25:
                    return False
                
                return True
            
            def _create_4channel_manual(self, mask, sample_info):
                """使用预验证的信息手动创建4通道标签"""
                mask_4ch = np.zeros((4, 512, 512), dtype=np.float32)
                
                # 使用预验证的实例信息
                instances = sample_info['instances_array']
                valid_instances = sample_info['valid_instances']
                
                if len(valid_instances) > 0:
                    # 通道0：中心距离
                    for instance_info in valid_instances:
                        instance_mask = instance_info['mask']
                        if np.any(instance_mask):
                            ...
                            if len(coords[0]) > 0:
                                ...
                                max_dist = distances[instance_mask].max()
                                if max_dist > 0:
                                    normalized_dist = 1.0 - (distances / max_dist)
                                    mask_4ch[0][instance_mask] = np.maximum(
                                        mask_4ch[0][instance_mask], 
                                        normalized_dist[instance_mask]
                                    )
                    ...
                    for instance_info in valid_instances:
                        instance_mask = instance_info['mask']
                        if np.any(instance_mask):
                            eroded = binary_erosion(instance_mask)
                            boundary = instance_mask & ~eroded
                            boundaries |= boundary
                    
                    if np.any(boundaries):
                        boundary_dist = distance_transform_edt(~boundaries)
                        mask_4ch[1] = 1.0 / (1.0 + boundary_dist * 0.1)
                
                # 通道2：前景
                mask_4ch[2] = (mask > 0).astype(np.float32)
                
                # 通道3：实例（归一化）
                if instances.max() > 0:
                    mask_4ch[3] = instances.astype(np.float32) / instances.max()
                
                return mask_4ch
            
            def _get_backup_sample(self):
                """获取备用样本，确保有前景对象"""
                # 创建一个简单的测试样本
                ...
                mask_4ch[0][circle_mask] = 1.0  # 中心距离
                mask_4ch[1][circle_mask] = 0.8  # 边界距离
                mask_4ch[2][circle_mask] = 1.0  # 前景
                mask_4ch[3][circle_mask] = 1.0  # 实例
                
                mask_tensor = torch.from_numpy(mask_4ch).float()
                
                return img_tensor, mask_tensor
        
        # 创建带shuffle属性和验证功能的数据加载器包装器
        class ShuffleDataLoader:
            """为micro_sam训练器添加shuffle属性和批次验证的DataLoader包装器"""
            def __init__(self, dataset, batch_size, shuffle, num_workers=0, drop_last=False, logger=None):
                ...
                self.shuffle = shuffle  # 添加shuffle属性
                self.dataset = dataset  # 添加dataset属性
                self.batch_size = batch_size  # 添加batch_size属性
                self.logger = logger  # 添加logger
            ...
            def __iter__(self):
                for batch_idx, batch_data in enumerate(self.dataloader):
                    ...
                    # 验证批次数据
                    try:
                        img_batch, mask_batch = batch_data
                        
                        # 检查批次中每个样本的前景
                        valid_batch = True
                        for sample_idx in range(img_batch.shape[0]):
                            sample_mask = mask_batch[sample_idx]
                            
                            # 检查前景通道（通道2）
                            foreground_channel = sample_mask[2]
                            foreground_pixels = torch.sum(foreground_channel > 0).item()
                            
                            if foreground_pixels < 25:
                                if self.logger:
                                    self.logger.log_warning(f"Batch {batch_idx}, sample {sample_idx} has insufficient foreground: {foreground_pixels} pixels")
                                valid_batch = False
                                break
                        ...
                        if valid_batch:
                            ...
                            if self.logger and self.failed_batches % 100 == 0:
                                failure_rate = self.failed_batches / self.total_batches * 100
                                self.logger.log_warning(f"Batch failure rate: {failure_rate:.2f}% ({self.failed_batches}/{self.total_batches})")
                        ...
                        if self.logger:
                            self.logger.log_error(f"Error validating batch {batch_idx}: {e}")
                        self.failed_batches += 1
                        continue
        ...
        # 创建数据集
        self.logger.log_info("Creating training dataset with strict validation...")
        train_dataset = MicroSAM4ChannelDataset(patch_infos_train, self.logger, is_train=True)
        
        self.logger.log_info("Creating validation dataset with strict validation...")
        val_dataset = MicroSAM4ChannelDataset(patch_infos_val, self.logger, is_train=False)
        
        if len(train_dataset) < 10 or len(val_dataset) < 2:
            raise ValueError(f"Insufficient data: train={len(train_dataset)}, val={len(val_dataset)}")
        
        # 使用ShuffleDataLoader包装器创建数据加载器 - 关键修正：传递logger
        train_loader = ShuffleDataLoader(
            train_dataset, 
            batch_size=batch_size, 
            shuffle=True, 
            num_workers=0,
            drop_last=True,
            logger=self.logger  # 关键修正：添加logger参数
        )
        
        val_loader = ShuffleDataLoader(
            val_dataset, 
            batch_size=1, 
            shuffle=False, 
            num_workers=0,
        ...
        # 验证数据格式
        self.logger.log_info("Validating 4-channel dataloader format...")
        ...
        if len(mask_batch.shape) == 4 and mask_batch.shape[1] == 4:
            self.logger.log_info("✓ Correct 4-channel mask format for segmentation decoder")
            
            # 检查各通道的内容
            for i in range(4):
                channel_data = mask_batch[:, i, :, :]
                non_zero_count = (channel_data > 0).sum().item()
                self.logger.log_info(f"  Channel {i}: range [{channel_data.min():.3f}, {channel_data.max():.3f}], non-zero: {non_zero_count}")
            
        else:
            raise ValueError(f"Expected 4-channel mask (B, 4, H, W), got shape: {mask_batch.shape}")
        
        # 验证符合micro_sam要求
        if img_batch.min() < 0 or img_batch.max() > 255:
            raise ValueError(f"Image range [{img_batch.min()}, {img_batch.max()}] outside [0,255]")
        
        self.logger.log_info("✓ 4-channel dataloader validation passed - compatible with micro_sam")
        self.logger.log_info(f"✓ Training samples validated: {len(train_dataset)}")
        self.logger.log_info(f"✓ Validation samples validated: {len(val_dataset)}")
        
        return train_loader, val_loader    
    
    
    def _create_microsam_fallback_dataloaders(self, train_imgs, train_masks, val_imgs, val_masks, batch_size):
        """创建专门兼容micro_sam的数据加载器，确保数据在[0,255]范围"""
        self.logger.log_info("Creating micro_sam compatible fallback dataloaders...")
        
        from torch.utils.data import DataLoader, Dataset
        import torch
        
        class MicroSAMDataset(Dataset):
            """专门为micro_sam设计的数据集类，确保数据范围正确"""
            ...
            def __init__(self, img_paths, mask_paths, logger):
                ...
                self.logger.log_info(f"MicroSAMDataset: {len(self.valid_indices)}/{len(img_paths)} valid")
            ...
            def __getitem__(self, idx):
                ...
                try:
                    ...
                    # 关键修复：确保图像在[0,255]范围（micro_sam要求）
                    # 检查当前数据范围
                    if img.max() <= 1.0:
                    ...
                    # 返回正确范围的零张量
                    return (torch.ones(1, 512, 512, dtype=torch.float32) * 128.0,  # 中间值
                        torch.zeros(512, 512, dtype=torch.long))
        
        # 创建数据集
        train_dataset = MicroSAMDataset(train_imgs, train_masks, self.logger)
        val_dataset = MicroSAMDataset(val_imgs, val_masks, self.logger)
        
        if len(train_dataset) < 10 or len(val_dataset) < 2:
            raise ValueError(f"Insufficient data: train={len(train_dataset)}, val={len(val_dataset)}")
            # 创建带shuffle属性的数据加载器包装器
        class ShuffleDataLoader:
            """为micro_sam训练器添加shuffle属性的DataLoader包装器"""
        ...
        try:
            ...
            self.logger.log_info(f"Fallback dataloader validation successful:")
            ...
            if img_batch.max() <= 1.0:
                raise ValueError("Image data in [0,1] range, micro_sam requires [0,255]")
            
            self.logger.log_info("✓ Data range validation passed - compatible with micro_sam")
            
        except Exception as e:
            raise Exception(f"Dataloader validation failed: {e}")
        
        return train_loader, val_loader
        
    def create_dataloaders(self, batch_size=4, num_workers=2):
        """创建符合micro_sam官方要求的数据加载器，修复数据范围问题"""
        ...
        for i, patch_info in enumerate(self.train_patches):
            try:
                ...
                if (len(img.shape) == 2 and len(mask.shape) == 2 and 
                    img.shape == mask.shape == (512, 512) and
                    np.sum(mask > 0) >= 50 and len(np.unique(mask)) >= 2):
        ...
        self.logger.log_info(f"Valid patches: train={len(self.train_patches)}, val={len(self.val_patches)}")
        
        if len(self.train_patches) < 20 or len(self.val_patches) < 5:
            raise ValueError(f"Insufficient data: train={len(self.train_patches)}, val={len(self.val_patches)}")
        
        # 使用 micro_sam 专用的备用数据加载器
        return self.create_official_dataloaders(
        valid_train_patches, 
        valid_val_patches, 
        batch_size)
...
class EnhancedMicroSAMTrainer:
    ...
    def init_wandb(self, model_name, epochs, learning_rate, batch_size, n_objects_per_batch, 
               train_size, val_size, model_type="vit_l_lm"):
...
def main():
    ...
    def get_model_config(model_type):
        ...
        return configs.get(model_type, configs["vit_l_lm"])
    ...
    try:
        ...
        # Phase 2: 创建数据加载器
        logger.log_info("Phase 2: Creating micro_sam compatible data loaders")
