...
class ResultVisualizer:
    ...
    def visualize_comparison(
        ...
        - Bottom-left: Prediction overlay (with high contrast colors)
        - Bottom-right: Foreground vs Background overlay
        """
        try:
            ...
            # Prediction overlay with high contrast colors (Bottom-left)
            ...
            # Foreground vs Background overlay (Bottom-right)
            axes[1, 1].imshow(image, cmap='gray')
            
            # 创建二值前景掩码
            foreground_mask = (prediction > 0).astype(np.float32)
            
            if np.any(foreground_mask):
                # 使用红色显示前景区域
                # 创建RGBA颜色映射：背景透明，前景红色
                from matplotlib.colors import ListedColormap
                colors = [(0, 0, 0, 0), (1, 0, 0, 0.6)]  # 透明背景，半透明红色前景
                cmap_fg_bg = ListedColormap(colors)
                axes[1, 1].imshow(foreground_mask, cmap=cmap_fg_bg, interpolation="nearest")
            
            axes[1, 1].set_title("Foreground vs Background")
            axes[1, 1].axis("off")
...
def create_random_colormap(n_labels):
    """为每个标签创建随机颜色图，用于可视化分割结果，确保颜色差异明显"""
    if n_labels <= 0:
    ...
    if n_labels == 1:
        colors = [(1.0, 0.0, 0.0)]  # 单个实例用红色
    else:
        # 在HSV空间中均匀分布色相，确保高对比度
        ...
        for i, hue in enumerate(hues):
            # 使用高饱和度和亮度确保颜色鲜明
            saturation = 0.8 + 0.2 * np.random.random()  # 0.8-1.0
            value = 0.7 + 0.3 * np.random.random()  # 0.7-1.0
...
class BatchEvaluationConfig:
    ...
    def __init__(self):
        ...
        self.cache_root = Path("/LD-FS/home/yunshuchen/DeepMicroSeg/microsam/Retrain_Evaluation/micro_sam_cache")
        ...
        self.models = [
            ...
            },
            {
                ...
                "checkpoint_path": "/LD-FS/home/yunshuchen/micro_sam/optimized_training_results/checkpoints/checkpoints/microsam_optimized_patches/best.pt"
                        ...
                        {
                "name": "base_all",  # 你的自定义模型
                "model_type": "vit_b_lm",  # 基础模型类型
                "checkpoint_path": "/LD-FS/home/yunshuchen/DeepMicroSeg/microsam/Retrain_Evaluation/micro_sam_cache/Training/Training_customize_vit_b_lm_vit_b_lm_20250609_071342/checkpoints/checkpoints/customize_vit_b_lm/best.pt"
            },
                        {
                "name": "large_all",  # 你的自定义模型
                "model_type": "vit_b_lm",  # 基础模型类型
                "checkpoint_path": "/LD-FS/home/yunshuchen/DeepMicroSeg/microsam/Retrain_Evaluation/micro_sam_cache/Training/Training_customize_vit_l_lm_vit_l_lm_20250608_164751/checkpoints/checkpoints/customize_vit_l_lm/best.pt"
            }
        ]
...
def process_dataset_worker(args):
    """Worker函数处理单个数据集 - 移除overlay保存功能，只保留visualization"""
    ...
    try:
        ...
        if results:
            ...
            if config.enable_visualization and visualization_candidates:
                ...
                random.seed(getattr(config, 'visualization_seed', 42))
                np.random.seed(getattr(config, 'visualization_seed', 42))
...
def main():
    ...
    # 设置数据目录 - 根据实际情况修改
    base_data_dir = "/LD-FS/home/yunshuchen/DeepMicroSeg/microsam/Retrain_Evaluation/micro_sam_cache/LD_patch_0520"
